#!/bin/bash
# lean 主线一键重跑：s04_rubric → s11_diagnose → s11b_remedy → s04b_split → s04c_severity → 导出
...
echo "[2/7] 准则直出 (s04_rubric.py)..."
python3 stages/s04_rubric.py
echo "  ✓ data/s04_rubric.jsonl"
echo

# ---- 3. RIFT 诊断 ----
echo "[3/7] RIFT 诊断 (s11_diagnose.py)..."
...
echo "[6/7] 负项分级 + veto 标记 (s04c_severity.py)..."
