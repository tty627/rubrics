...
from lib import stage, dimensions, rubric, task_input
...
【总量范围】通常 {N_MIN}-{N_MAX} 条（含扣分项），上限不得超过 {N_MAX} 条。
题目简单、要求单一或证据不足时可以少于 {N_MIN} 条，不为凑数新增内容。

【准则的来源与判据】
先按以下优先级确定评分内容：
1. 题目明确要求的任务、子问题、格式和限制；
2. 原始 system/developer/user 消息明确给出的事实、条件和限制；
3. 完成该任务不可缺少、且不依赖个人偏好的通用质量条件。

每条准则都要能回指以上来源。intent、候选评价轴和模型常识只是整理工具，
不是新增事实来源；它们与原始任务冲突或缺少依据时不得写成硬要求。
不要把「理想答案还可以写什么」当成「用户要求必须写什么」。

【六条禁止】
1. **禁止写来源不明的应然事实**。具体答案、数值、日期、名单、比例、分类、
   专有名词、周期和机理，只能来自原始任务明确提供的信息；不得用模型记忆补全。
   题目要求回答者检索最新外部事实、但输入未提供事实值时，准则应检查来源、
   统计口径、时间点和推导是否可核验，不得预先写死模型猜出的值。
   涉及条件依赖的结论，若题目未给出前提，不要写。
...
4. **禁止写「与标准答案一致」这类空壳**。判分器手上没有标准答案，执行不了。
   ...
   ❌ "最终答案与标准答案一致"        ← 判分器无从核对
   ✅ "最终答案为 λ=√2"
   只有原始任务明确给出的答案才能直接写入准则。
   没有可靠答案时，不要猜；改为检查推导依据、核验方法或来源口径。
...
动词本身不决定准则质量。列举、描述、比较、分析在题目明确要求这些任务时可以使用；
关键是写清可观察的完成条件，并且不把未提供的具体结论冒充标准答案。
...
【扣分项】允许 0-2 条。只有题目中存在明确、独立且出现后应额外扣分的错误模式时才写；
不要为了凑数量构造扣分项。遗漏正向要求通常由正向项不得分处理，不再镜像扣分。
直接写错误现象本身，is_positive 填 false；描述必须是「出现了什么错误」，
不能把「未犯错误」「拒绝违规内容」等正确行为写成负项。

⚠️ **扣分项禁止编造具体细节**：
- 只有原始任务明确描述了错误模式时，才可写对应的具体扣分项
- 否则只写题面可推出的通用错误类型，不要编造具体数值/字节/参数

  错误示例（verifiable 题未给出错误模式时）：
  ❌ "将十六进制'52'错误解码为字符'S'"  ← 编造了具体字节和错误结果
  ✅ "解码后出现错误字符，且无法还原题目要求的文本"  ← 可观察错误
...
def build(r):
    ...
    # 候选回答与 rubric 生成隔离：ref_responses / ref_errors 不进入此 prompt。
    # 错误回答只能在 rubric 冻结后用于 judge 或实测，不得反向定义评分标准。

    user = (f'【学科】{subj}\n'
            f'【提问意图】{r.get("intent", "")}\n'
            f'【题型】{r.get("question_type", "")} → {form}{gate}\n\n'
            f'{task_input.prompt_context(r, question=q, max_chars=10000)}\n\n'
...
def anchor_block(r):
    """Deprecated compatibility helper; candidate answers stay outside generation."""
    return ''
...
# 「可核验字面量」与「主观程度词」的词表在 lib/rubric，阶段 10 卡 veto 门槛时
# 要用同一套口径 —— 两边各写一份必然漂移。
...
TOPIC_VERB = re.compile(r'^(列举|列出|描述|比较|分析|涉及|覆盖|说明了)'
                        r'|至少\s*\d+\s*[种个条项]'
                        r'|^正确描述|^简要说明')
# 扣分项的主观阈值词（同上，口径在 lib/rubric）
SUBJ_DEG = rubric.SUBJ_DEG
