#!/usr/bin/env python3
"""阶段 27：导出导师指定 schema 的 rubrics（交付档 / 内部档）。

用法（源 = 阶段 26 的交付源，见 pipeline/03_run_response_evaluation.sh）：
  python3 pipeline/27_export_rubric_delivery.py --src ... --out ...           # 交付档
  python3 pipeline/27_export_rubric_delivery.py --src ... --out ... --full    # 内部档

两种模式都只写 `--out` 一个文件：不带 --full 出交付档（严格白名单字段），
带 --full 出内部档（额外带血缘 + 诊断 + 质量标记）。runner 调两次、给不同 --out。
...
def main():
    ...
    # 默认源 = 阶段 26 的交付源（流水线末端）。指向中间步会静默丢掉后续产出
    # （2026-08-13 交付档一条没过 RIFT、2026-08-14 severity/veto 全空，都是这个坑）。
    ap.add_argument('--src', default=str(paths.EVALUATION_FILES['delivery_source']),
                    help='源 jsonl，默认阶段 26 的交付源（已过 RIFT 诊断处置 + '
                         '缺陷重写 + 负项分级 + 实测处置选优）')
    ap.add_argument('--out', default=str(paths.DELIVERY_FILES['rubric']))
    ap.add_argument('--full', action='store_true',
                    help='把 --out 写成内部档（带血缘/诊断/质量标记）而非交付档')
    a = ap.parse_args()

    src = a.src if os.path.isabs(a.src) else os.path.join(REPO, a.src)
    if not os.path.exists(src):
        sys.exit(f'缺少 {src}\n'
                 f'先跑 bash pipeline/00_run_all.sh（或至少跑到阶段 26）')
    ...
    if a.full:
        # 内部档写 --out 本身，不另起文件名。此前 --full 把带血缘的记录写到硬编码的
        # `rubrics_internal.jsonl`，而 --out 拿到的仍是精简档 —— runner 用
        # `--out .../rubric_internal.jsonl --full` 调用时，那个路径下落的是**精简档**，
        # 血缘字段（硬约束第 4 条）静默丢失，manifest 与 lib/paths 指的都是这条路径。
        with open(a.out, 'w', encoding='utf-8') as f:
            for r in internal:
                f.write(json.dumps(r, ensure_ascii=False) + '\n')
        print(f'已写出 {a.out}（含血缘/诊断/质量标记）')
