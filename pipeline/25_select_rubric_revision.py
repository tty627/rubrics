...
输入: 各轮阶段 23 的区分度诊断（RP_S11LE_ROUNDS，逗号分隔，先基线后各轮）
输出: RP_S11LE_OUT（runner 给 evaluation/25_selected_rubrics.jsonl）
"""
import json, os, sys
...
ROUNDS = [x.strip() for x in os.environ.get(
    'RP_S11LE_ROUNDS',
    'evaluation/23_discrimination_diagnostics.jsonl,'
    'evaluation/23_discrimination_diagnostics_r01.jsonl,'
    'evaluation/23_discrimination_diagnostics_r02.jsonl,'
    'evaluation/23_discrimination_diagnostics_r03.jsonl').split(',') if x.strip()]
...
def cost(f):
    """越小越好 = 缺陷数。skip 记 1.5，落在「1 个确诊缺陷」与「2 个」之间。

    这个位置是刻意的，别调成 0.5：skip 意味着这一轮没测出结论，选它等于交付一份
    质量未知的 rubric。已知只有 1 个缺陷的版本反而更可交付 —— 缺陷记在 `_s11Le`
    的 residual 里，下游看得见；skip 版本下游只知道「不知道」。所以只有当另一版
    烂到 2 个以上缺陷时，才宁可要测量受限的那版。
    """
    if f == {'skip'}:
        return 1.5
    return len(f)
