...
**veto 门槛（prompt 写清楚 + 代码兜底）**：
  ...
  3. 判定线清晰：复用 lib/rubric 的 SUBJ_DEG/ANCHOR，主观程度词不给 veto
...
# 判定线词表与阶段 06 的质量标记同源，口径只允许有一份（见 lib/rubric）。
SUBJ_DEG, ANCHOR = rubric.SUBJ_DEG, rubric.ANCHOR
...
【severity 分级】
- principle（原则性错误）：该错误直接破坏题目核心任务，使回答的主要结论不可用，
  或直接执行题目明确禁止且不可接受的行为。不要因为措辞偏好、局部遗漏、观点差异、
  诚实披露身份或可修正的小错判为 principle。
- major（较严重）：明显扣分、但不至于整题作废的错误。
- minor（轻微）：局部小瑕疵、表述不严谨。

【veto 门槛 —— 必须同时满足以下全部条件，才给 is_veto=true】
1. **原子**：这条只描述一个单一错误，不含「且/或/并且/同时」串接多个错误。
2. **判据是该错误使题目核心任务整体失败**，不是「扣分较多」或「不符合偏好」。
   只有 principle 级错误能 veto（major/minor 一律 is_veto=false）。
