...
判定取闭集五类；模型返回越界或缺字段时本阶段失败，不以默认标签继续。
"""
import os, re, sys
...
def main():
    ...
    recs = stage.read_jsonl('tasks/01_task_dataset.jsonl')
    ...
    def work(r):
        ...
        if v not in VERDICTS:
            raise ValueError(f'{r["rid"]} verdict 越界: {v!r}')
        rw = (obj.get('rewritten') or '').strip()
        if v.startswith('改写') and not rw:
            raise ValueError(f'{r["rid"]} 要求改写但 rewritten 为空')
    ...
    out, errors = stage.run(work, recs, workers=WORKERS, desc='stage-02')
    stage.require_success(errors, 'Stage 02')
    stage.stat_cached([r.pop('_meta') for r in out])
    stage.write_jsonl('tasks/02_filtered_tasks.jsonl', out)
