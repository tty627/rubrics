"""步骤 11：RIFT 免池诊断（lean 流程版）—— 适配 s04_rubric.jsonl。

与 s11_diagnose.py 的差异：
1. 输入: s04_rubric.jsonl（准则在 rubrics[] 里，schema 已规整）
2. 输出: s11_diagnosed.jsonl（在原记录上添加 diagnoses[] 字段）
...
SRC = os.environ.get('RP_S11L_SRC', 's04_rubric.jsonl')
...
def main():
    ...
    print(f'步骤 11 RIFT 免池诊断: {len(recs)} 条, 源={SRC}, 诊断器={m.name}, thinking={THINK}')
