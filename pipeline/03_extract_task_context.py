...
def main():
    ...
    def work(r):
        ...
        ic = obj.get('explicit_constraints') or {}
        if not isinstance(ic, dict):
            raise ValueError(f'{r["rid"]} explicit_constraints 不是对象')
        return {**r, 'intent': str(obj.get('intent', ''))[:120],
                'explicit_constraints': {k: str(ic.get(k, ''))[:80]
    ...
    print(f'  风险边界非空  : {sum(1 for r in out if r["explicit_constraints"]["risk_boundary"] not in ("", "未指定"))}')
    print('\n  抽样两条：')
    for r in out[:2]:
