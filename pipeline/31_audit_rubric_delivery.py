...
用法：
  python3 pipeline/31_audit_rubric_delivery.py                        # 审计当前交付档
  python3 pipeline/31_audit_rubric_delivery.py 新.jsonl --base 旧.jsonl  # 两版对比

这是交付档质量的唯一审计入口（原 tests/test_s04_flags.py 只查 3 类、且写死了
「修复前=交付档」的比较对象，已删除）。
...
判定用的正则直接从阶段 06 的护栏引，避免护栏和审计各写一套导致漂移。
"""
import argparse
import json
import os
...
from lib import rubric, stage

# 复用阶段 06 护栏的判定口径：审计与生成期用同一套正则，否则「审计通过」
# 和「生成期没打标」会各说各话。
_gen = stage.load('06_generate_rubric.py')
VAGUE, ANCHOR, REF_ONLY = _gen.VAGUE, _gen.ANCHOR, _gen.REF_ONLY
BULK, MENTION, SUBJ_DEG = _gen.BULK, _gen.MENTION, _gen.SUBJ_DEG
...
def main():
    ...
    ap.add_argument('path', nargs='?',
                    default=os.path.join(REPO, 'outputs', 'current',
