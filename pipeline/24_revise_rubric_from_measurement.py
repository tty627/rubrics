...
# 锚可达性探针要用阶段 22 的判分口径（同一套 SYS + 同一套准则渲染），
# 换口径测出来的分数与阶段 23 的地板判定不可比。
s22 = stage.load('22_score_response_pool.py')
...
SYS_ANCHOR = '''你在回答一道题。请给出你能给出的最好回答：
内容正确、覆盖完整、论证充分、篇幅充实。直接作答，不要说明你在做什么。'''


def anchor_reachable(m_ground, m_judge, r):
    """用权威模型现场作答当锚，探测 rubric 是否可达。返回 (是否可达, 得分率)。

    地板信号（各档全线低分）有两种成因：rubric 真过严，或回复池造得不行。
    区分办法是拿一份独立于回复池的高质量回答去打分——**锚能拿到分就说明准则可满足**，
    地板来自 pool 侧，此时"放松准则"会把本来好用的 rubric 改坏（388 全量实测：
    8 道复测后仍地板的题里 6 道锚拿到 42%~100%）。

    锚必须独立于被测的 strong 档，否则探测是循环论证；也不能用题目自带的候选回答
    （那是待评对象，且并非每题都有）。故由 grounder 现场生成。
    判分口径复用 22 的 build —— 换口径测出来的分与地板判定不可比。
    """
    rubrics = r.get('rubrics') or []
    if not rubrics:
        return False, None
    q = (r.get('query_eff') or r.get('question') or '')[:2000]
    text, _ = stage.llm.call(m_ground,
    ...
    if not text:
        return False, None
    obj, _ = stage.json_call(m_judge, judge_msgs(r, text, rubrics),
...
def judge_msgs(r, text, rubrics):
    """复用阶段 22 的判分口径，保持锚可达性与实测得分可比。"""
    return s22.build(r, {'text': text}, rubrics)
...
def main():
    ...
    m_ground = stage.pick('RP_M_GROUND', 'grounder')
    m_judge = stage.pick('RP_M_JUDGE', 'judge')
    recs = stage.read_jsonl(SRC)
    print(f'步骤 24 实测处置: {len(recs)} 题, 源={SRC}')
    print(f'  重写模型={m.name}  锚={m_ground.name}  判分={m_judge.name}  '
          f'白名单={"(" + ",".join(ONLY) + ")" if ONLY else "默认启发式"}')
    ...
    def one(rid):
        ...
        if action == 'floor':
            ...
            # 权威模型现场作答能拿到分 → 准则可满足，地板来自 pool 构造，不放松 rubric
            reach, best = anchor_reachable(m_ground, m_judge, r)
            if reach:
                return rid, None, f'权威锚在本 rubric 上得 {best:.0%}，准则可达到', 'anchor_ok'
