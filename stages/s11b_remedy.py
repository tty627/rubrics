"""步骤 11b：诊断处置（lean 流程版）—— 按失效模式分级处置。

输入: s11_diagnosed.jsonl
输出: s11b_remedied.jsonl + _defect_queue.jsonl（待拆队列，供 s04b_split 消费）
...
处置策略（按失效模式分级，优先级 factual > drop > split）：
  ...
  non-atomic  → **不删**，落 _defect_queue.jsonl 等 s04b_split 拆分。
...
SRC = os.environ.get('RP_S11LB_SRC', 's11_diagnosed.jsonl')
...
def gate_cids(r):
    ...
    可能不止一条：s04b_split 会把「全部/每一个都对」式的悬崖项拆成「规则对不对」+
    「条目全不全」两条各占一半分值，两条合起来才是答案判据，都要受豁免保护。
    判定口径 = lib/rubric.gate_indices（与 export 的 is_gate、s12_judge 的答案项
...
def main():
    ...
    print(f'步骤 11b 诊断处置: {len(recs)} 条, 源={SRC}')
    ...
    stage.write_jsonl('s11b_remedied.jsonl', res)
